...
from safer_streets_tooling.beahiv_grid import ORIENTATION, SIDE_LENGTH
from safer_streets_tooling.transform import beahiv, beahiv_counts
...
def test_spatial_id_is_a_signed_integer_on_the_declared_grid():
    """spatial_id is a BIGINT that decodes straight back to the configured side length/orientation.

    beahiv's reserved top bits are what make this possible: every id is below 2**61, so no cast, no
    unsigned column and no hex-string encoding stands between the table and ``decode``.
    """
    con = _connect()
    _crime_data(con)
    ...
    for (cell,) in con.execute(f"SELECT spatial_id FROM {TABLE}").fetchall():
...
def test_build_is_idempotent():
    """Rebuilding re-registers the UDFs rather than failing on the name already being in the catalog."""
    con = _connect()
    _crime_data(con)
...
def test_udf_matches_beahiv_across_a_multi_vector_scan():
    ...
    mismatches = con.execute(f"""
        ...
        ) WHERE {beahiv.ENCODE_UDF}(x, y) != beahiv_cell_scalar(x, y)
    """).fetchone()
    assert mismatches == (0,)
