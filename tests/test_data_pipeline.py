...
def test_food_outlets_extracts_england_and_wales_food_venues(tmp_path, monkeypatch):
    ...
    assert {"address", "structural_score"}.isdisjoint(cols)  # address + structural dropped (postcode, hygiene kept)
...
def test_download_fhrs_uses_cache(tmp_path, monkeypatch):
    monkeypatch.setattr(_common, "data_dir", lambda: tmp_path)
