...
is named from that unit's key — ``hotspots`` (the extract itself), ``hotspots_geogs`` /
``hotspots_lad24cd_lookup`` (the lookups keyed *on* the unit) and ``crime_counts_hotspots`` /
``building_counts_hotspots`` (the counts aggregated *onto* it) — so the key alone identifies the whole
family, and a hotspot step added later is covered the day it is written without touching this module.
...
def is_local_only(name: str) -> bool:
    ...
    Matches the bare key, the ``{key}_`` prefix and the ``_{key}`` suffix, which between them cover every
    way a relation is named after its spatial unit. Deliberately broad: a table that merely looks like it
    belongs to the family is held back rather than published.
    """
