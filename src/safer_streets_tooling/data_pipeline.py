...
Both then write ``index.parquet`` (also available standalone as ``index``) — a catalogue with one row
per extract + transform table (phase, name, description, schema; see ``safer_streets_tooling.index``).
...
def run_index(edir: Path, tdir: Path, out_path: Path, resolutions: list[int]) -> None:
...
def index(resolutions: list[int] = [8, 9, 10]) -> None:  # noqa: B006
    ...
    registry description, row/column counts, geometry flag and column list. Regenerated by ``assemble``
    and ``build``; run standalone to refresh it after extracting/transforming a subset.
    """
