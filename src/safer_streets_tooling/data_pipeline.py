...
The pipeline has three phases (extract → transform → load):
  ...
  3. **load**       *(optional)* a minimal consumer database is assembled from the transform parquet —
     ...
     and the schools / poi / naptan / food_outlets / imd_scores_pct / land_cover / oac (+ oac_classification) feature layers —
...
def _minimal_tables(resolutions: list[int]) -> list[str]:
    ...
    - the ``DEFAULT_FEATURE_TABLES`` feature layers (schools / poi / naptan / food_outlets / imd_scores_pct / land_cover / oac +
...
def run_load(
    ...
    ``DEFAULT_FEATURE_TABLES`` feature layers (schools / poi / naptan / food_outlets / imd_scores_pct / land_cover / oac (+ oac_classification), under ``edir``) are
...
def load(
    ...
    and the schools / poi / naptan / food_outlets / imd_scores_pct / land_cover / oac (+ oac_classification) feature layers are imported. ``--include NAME`` (repeatable)
