...
a one-line ``description``, its ``n_rows`` / ``n_columns`` / ``columns`` schema summary and whether it
carries geometry — so a consumer can discover the build outputs without opening every file.
...
_COLUMNS = ["phase", "name", "description", "n_rows", "n_columns", "has_geometry", "columns"]
